...
export async function GET(
  ...
  const blocked = await guardPublicToken('proposta-usina-pdf')
  ...
  try {
    ...
    logger.error('proposta-usina/pdf', 'Falha ao renderizar PDF', err, {
  ...
  try {
    ...
    logger.error('proposta-usina/pdf', 'PDF gerado mas não armazenado', err, { token: token.slice(0, 8) })
  }
